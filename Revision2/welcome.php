...
if ($first_name == NULL || $last_name == NULL || $city == NULL || $states == NULL || $zipcode == NULL || $phone_number == NULL || strlen($phone_number) != 10 || $email == NULL || $password == NULL || $password != $confirm ){
	...
	?><script>window.alert("invalid password");</script> <?php
	include('register.html');
    die();
}

$customerQuery = "insert into customer (first_name, last_name, city, states, zipcode, phone_number , email, pass ) values ('$first_name', '$last_name', '$city', '$states', $zipcode, '$phone_number', '$email', '$password')";
exec($customerQuery);
